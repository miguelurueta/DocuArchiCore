...
try {
    ...
    Run-Git -RepoRoot $satelliteRoot -CliArgs @("checkout", "-b", "scrum-86-ocupado")
    Add-Content -Path (Join-Path $satelliteRoot "service.txt") -Value "`nworking tree busy"
    ...
    function global:Invoke-RestMethod {
        ...
        if ($Method -ieq "Get" -and $Uri -like "*/rest/api/3/issue/*?fields=summary,description,status") {
            ...
            return @{
                fields = @{
                    summary = "Implementacion worktree orchestration"
                    description = @{
                        type = "doc"
                        content = @(
                            @{
                                type = "paragraph"
                                content = @(
                                    @{ type = "text"; text = "Impacts MiApp.Services and needs orchestrated repo management." }
                                )
                            }
                        )
                    }
    ...
    try {
        ...
        try {
            ...
            if (-not (Test-Path $worktreePath)) {
                throw "Expected worktree path not found: $worktreePath"
            }

            $reasons = @($metadata.checkoutState.reasons)
            if ($reasons -notcontains "branch_in_use") {
                throw "Expected branch_in_use reason in metadata. Actual: $($reasons -join ', ')"
            }
            if ($reasons -notcontains "dirty_worktree") {
