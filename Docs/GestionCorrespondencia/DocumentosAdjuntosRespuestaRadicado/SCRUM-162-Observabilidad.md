...
## logs agregados
- controller: inicio de solicitud con `idRespuestaRadicado` y alias
...
## estructura de logs
Mensajes estructurados con placeholders (`{idRespuestaRadicado}`, `{Alias}`, `{Rows}`, `{Ms}`).
...
- idSolicitudAprobacion equivalente (`idRespuestaRadicado`): sí
...
## troubleshooting
- sin resultados: revisar existencia de `idRespuestaRadicado` y datos en `id_imagen_gabinete`
...
## ejemplos de logs
- `SolicitaDocumentosAdjuntosRespuestaRadicado: idRespuestaRadicado=1250 alias=WF`
- `SolicitaDocumentosAdjuntosRespuestaRadicado: idRespuestaRadicado=1250 alias=WF rows=48 ms=22`
