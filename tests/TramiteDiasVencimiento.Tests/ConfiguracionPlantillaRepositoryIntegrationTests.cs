...
public sealed class ConfiguracionPlantillaRepositoryIntegrationTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task SolicitaConfiguracionPlantillaAsync_CuandoExisteRegistro_RetornaOk()
    {
        if (_dockerUnavailable)
        {
            return;
        ...
        var repository = new ConfiguracionPlantillaRepository(dapper);

        var result = await repository.SolicitaConfiguracionPlantillaAsync(67, 1, "DA");
    ...
    [Fact]
    public async Task SolicitaConfiguracionPlantillaAsync_CuandoNoExisteRegistro_RetornaSinResultados()
    {
        if (_dockerUnavailable)
        {
            return;
        ...
        var result = await repository.SolicitaConfiguracionPlantillaAsync(999, 9, "DA");
