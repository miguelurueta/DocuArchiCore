...
Use this template at the start of each Jira ticket (`SCRUM-121`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-121`
- Summary: `ACTUALIZACION-AG-GRID-CELL-FIJA`
- Coordinator change: `openspec/changes/scrum-121-actualizacion-ag-grid-cell-fija/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/152 | pending | in_review |
...
Run `opsxj:new SCRUM-121` only in rows with `Impacta? = yes`.
