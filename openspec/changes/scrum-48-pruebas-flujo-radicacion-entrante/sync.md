...
Use this template at the start of each Jira ticket (`SCRUM-48`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-48`
- Summary: `PRUEBAS-FLUJO-RADICACION-ENTRANTE`
- Coordinator change: `openspec/changes/scrum-48-pruebas-flujo-radicacion-entrante/`
...
Run `opsxj:new SCRUM-48` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-48` only after the repo PR is merged.
