...
Use this template at the start of each Jira ticket (`OPSXJ-777`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `OPSXJ-777`
- Summary: `Issue OPSXJ-777`
- Coordinator change: `openspec/changes/opsxj-777-issue-opsxj-777/`
...
Run `opsxj:new OPSXJ-777` only in rows with `Impacta? = yes`.
Run `opsxj:archive OPSXJ-777` only after the repo PR is merged.
