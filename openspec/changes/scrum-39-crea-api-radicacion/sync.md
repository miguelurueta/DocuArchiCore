...
Use this template at the start of each Jira ticket (`SCRUM-39`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-39`
- Summary: `CREA-API-RADICACION`
- Coordinator change: `openspec/changes/scrum-39-crea-api-radicacion/`
...
Run `opsxj:new SCRUM-39` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-39` only after the repo PR is merged.
