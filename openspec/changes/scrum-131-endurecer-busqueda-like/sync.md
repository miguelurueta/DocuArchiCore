...
Use this template at the start of each Jira ticket (`SCRUM-131`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-131`
- Summary: `ENDURECER-BUSQUEDA-LIKE`
- Coordinator change: `openspec/changes/scrum-131-endurecer-busqueda-like/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/171 | pending | in_review |
...
Run `opsxj:new SCRUM-131` only in rows with `Impacta? = yes`.
