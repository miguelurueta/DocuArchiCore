...
Use this template at the start of each Jira ticket (`SCRUM-124`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-124`
- Summary: `Exponer total real y conteo filtrado en workflowInboxgestion`
- Coordinator change: `openspec/changes/scrum-124-exponer-total-real-y-conteo-filtrado-en/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/158 | pending | in_review |
...
Run `opsxj:new SCRUM-124` only in rows with `Impacta? = yes`.
