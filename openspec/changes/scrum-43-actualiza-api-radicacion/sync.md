...
Use this template at the start of each Jira ticket (`SCRUM-43`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-43`
- Summary: `ACTUALIZA-API-RADICACION`
- Coordinator change: `openspec/changes/scrum-43-actualiza-api-radicacion/`
...
Run `opsxj:new SCRUM-43` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-43` only after the repo PR is merged.
