...
Use this template at the start of each Jira ticket (`SCRUM-134`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-134`
- Summary: `CORREGIR-SQL-BUQUEDA-LIKE`
- Coordinator change: `openspec/changes/scrum-134-corregir-sql-buqueda-like/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/177 | pending | in_review |
...
Run `opsxj:new SCRUM-134` only in rows with `Impacta? = yes`.
