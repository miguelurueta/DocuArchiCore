...
Use this template at the start of each Jira ticket (`SCRUM-120`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-120`
- Summary: `ACTUALIZACION-DynamicUiTableBuil-MenuItems`
- Coordinator change: `openspec/changes/scrum-120-actualizacion-dynamicuitablebuil-menuite/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/150 | pending | in_review |
...
Run `opsxj:new SCRUM-120` only in rows with `Impacta? = yes`.
