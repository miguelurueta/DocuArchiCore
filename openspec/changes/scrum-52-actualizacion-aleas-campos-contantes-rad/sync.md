...
Use this template at the start of each Jira ticket (`SCRUM-52`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-52`
- Summary: `ACTUALIZACION-ALEAS-CAMPOS-CONTANTES-RADICACION`
- Coordinator change: `openspec/changes/scrum-52-actualizacion-aleas-campos-contantes-rad/`
...
Run `opsxj:new SCRUM-52` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-52` only after the repo PR is merged.
