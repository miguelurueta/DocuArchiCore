...
Use this template at the start of each Jira ticket (`SCRUM-174`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-174`
- Summary: `ACTUALIZA-OPSXJ:NEW ORCHESTRATE`
- Coordinator change: `openspec/changes/scrum-174-actualiza-opsxj-new-orchestrate/`
...
| DocuArchiCore | yes | implementation_required | hardening de flujo opsxj para gates de tareas y revision OpenSpec | done | https://github.com/miguelurueta/DocuArchiCore/pull/211 | pending | in_review |
...
Run `opsxj:new SCRUM-174` only in rows with `Impacta? = yes`.
