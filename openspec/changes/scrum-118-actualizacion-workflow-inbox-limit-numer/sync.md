...
Use this template at the start of each Jira ticket (`SCRUM-118`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-118`
- Summary: `ACTUALIZACION-WORKFLOW-INBOX-LIMIT-NUMERO-TAREAS`
- Coordinator change: `openspec/changes/scrum-118-actualizacion-workflow-inbox-limit-numer/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/148 | pending | in_review |
...
Run `opsxj:new SCRUM-118` only in rows with `Impacta? = yes`.
