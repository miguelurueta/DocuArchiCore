...
Use this template at the start of each Jira ticket (`SCRUM-133`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-133`
- Summary: `PRUEBAS-REGRESION-BACKEND-AUTO-COMPLETE`
- Coordinator change: `openspec/changes/scrum-133-pruebas-regresion-backend-auto-complete/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/175 | pending | in_review |
...
Run `opsxj:new SCRUM-133` only in rows with `Impacta? = yes`.
