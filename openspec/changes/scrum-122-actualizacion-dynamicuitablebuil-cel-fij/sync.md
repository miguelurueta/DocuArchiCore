...
Use this template at the start of each Jira ticket (`SCRUM-122`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-122`
- Summary: `ACTUALIZACION-DynamicUiTableBuil-CEL-FIJA`
- Coordinator change: `openspec/changes/scrum-122-actualizacion-dynamicuitablebuil-cel-fij/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/153 | pending | in_review |
...
Run `opsxj:new SCRUM-122` only in rows with `Impacta? = yes`.
