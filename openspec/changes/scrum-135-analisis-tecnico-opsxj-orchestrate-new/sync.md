...
Use this template at the start of each Jira ticket (`SCRUM-135`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-135`
- Summary: `ANALISIS-TECNICO-opsxj:orchestrate:new`
- Coordinator change: `openspec/changes/scrum-135-analisis-tecnico-opsxj-orchestrate-new/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/179 | pending | in_review |
...
Run `opsxj:new SCRUM-135` only in rows with `Impacta? = yes`.
