...
Use this template at the start of each Jira ticket (`SCRUM-45`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-45`
- Summary: `VALIDACION-DIMENSION-CAMPOS-RADICACION`
- Coordinator change: `openspec/changes/scrum-45-validacion-dimension-campos-radicacion/`
...
Run `opsxj:new SCRUM-45` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-45` only after the repo PR is merged.
