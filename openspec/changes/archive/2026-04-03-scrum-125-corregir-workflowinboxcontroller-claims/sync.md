...
Use this template at the start of each Jira ticket (`SCRUM-125`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-125`
- Summary: `CORREGIR-WORKFLOWINBOXCONTROLLER-CLAIMS-REALES-Y-ELIMINAR-HARDCODES`
- Coordinator change: `openspec/changes/scrum-125-corregir-workflowinboxcontroller-claims/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/161 | pending | in_review |
...
Run `opsxj:new SCRUM-125` only in rows with `Impacta? = yes`.
