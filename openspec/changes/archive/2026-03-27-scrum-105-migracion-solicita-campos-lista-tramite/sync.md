...
Use this template at the start of each Jira ticket (`SCRUM-105`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-105`
- Summary: `MIGRACION-Solicita_campos_lista_tramite`
- Coordinator change: `openspec/changes/scrum-105-migracion-solicita-campos-lista-tramite/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/127 | pending | in_review |
...
Run `opsxj:new SCRUM-105` only in rows with `Impacta? = yes`.
...
## Migration Read-Only Repositories (MIGRACION-NET Migrar)
