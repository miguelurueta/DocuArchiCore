...
Use this template at the start of each Jira ticket (`SCRUM-53`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-53`
- Summary: `ACTUALIZACION-MENSAJE-VALIDACION-CAMPOS-RADICACION`
- Coordinator change: `openspec/changes/scrum-53-actualizacion-mensaje-validacion-campos/`
...
Run `opsxj:new SCRUM-53` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-53` only after the repo PR is merged.
