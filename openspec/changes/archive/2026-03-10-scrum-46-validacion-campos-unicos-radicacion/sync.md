...
Use this template at the start of each Jira ticket (`SCRUM-46`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-46`
- Summary: `VALIDACION-CAMPOS-UNICOS-RADICACION`
- Coordinator change: `openspec/changes/scrum-46-validacion-campos-unicos-radicacion/`
...
Run `opsxj:new SCRUM-46` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-46` only after the repo PR is merged.
