...
- Jira issue key: SCRUM-72
- Jira summary: CREA-REPOSITORY-SOLICITA-ESTRUCTURA-CONFIGURACIO-LISTADO-RUTA
- Jira URL: https://contasoftcompany.atlassian.net/browse/SCRUM-72
...
PROMPT ARQUITECTÓNICO — Consulta Configuración lista campos ruta Rol esperado: Arquitecto de software senior y desarrollador backend .NET Nombre: SolicitaEstructuraConfiguracionListadoRutaRepository OBJETIVO Implementar la consulta de configuración de plantilla de radicación siguiendo el patrón Repository dentro de un proyecto multirepo con arquitectura por capas. Se debe consultar la configuración de una plantilla de radicación usando: - idPlantilla - tipoRadicacionPlantilla - defaultDbAlias Parámetros: - idRuta - defaultDbAlias TABLA: configuracion_listado_ruta CONDICIONES Rutas_Workflow_id_Ruta = idRuta --------------------------
 UBICACIÓN DE COMPONENTES
 -------------------------- Repositorio Proyecto: MiApp.Repository Ruta: /Repositorio/Workflow/RutaTrabajo Modelo Proyecto: MiApp.Models Ruta: Models/Workflow/RutaTrabajo Documentación Repositorio: DocuArchiCore Ruta: /Docs/Workflow/RutaTrabajo ---------------------------
 FIRMA ESPERADA DEL MÉTODO
 --------------------------- Task<AppResponses<configuracioNlistadoruta?>> SolicitaEstructuraConfiguracionListadoRutaRepository( int idRuta, string defaultDbAlias) Si la tabla puede retornar múltiples registros válidos, usar: Task<AppResponses<List>> ----------------------------
 ...
 ----------------------- Ubicación: /Docs/workflow/RutaTrabajo Archivos: SCRUM-[ID]-Diagramas.md Debe incluir: - diagrama de casos de uso - diagrama de clases - diagrama de secuencia - diagrama de estados (si aplica) SCRUM-[ID]-Integracion-Frontend.md Debe incluir: - descripción técnica del DTO de respuesta - parámetros DOCUMENTAR CONSUMO INTERNO Ejemplo de invocación PRUEBAS UNITARIAS Incluir pruebas para: - respuesta exitosa con datos - respuesta exitosa sin resultados - excepción controlada - mock de repositorio o dependencias PRUEBAS DE INTEGRACIÓN Usar: MySQL Testcontainers / Docker Validar: - consulta real a la tabla - filtros correctos - comportamiento sin registros - comportamiento con registros válidos -------------
 ENTREGABLES
...
- Validar con openspec.cmd validate scrum-72-crea-repository-solicita-estructura-conf.