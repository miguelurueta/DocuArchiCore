...
Use this template at the start of each Jira ticket (`SCRUM-37`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-37`
- Summary: `CREA-API-LISTA-COINSIDENCIA-RADICADOS`
- Coordinator change: `openspec/changes/scrum-37-crea-api-lista-coinsidencia-radicados/`
...
Run `opsxj:new SCRUM-37` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-37` only after the repo PR is merged.
