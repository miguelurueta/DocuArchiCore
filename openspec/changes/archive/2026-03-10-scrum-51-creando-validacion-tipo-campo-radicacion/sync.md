...
Use this template at the start of each Jira ticket (`SCRUM-51`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-51`
- Summary: `CREANDO-VALIDACION-TIPO-CAMPO-RADICACION`
- Coordinator change: `openspec/changes/scrum-51-creando-validacion-tipo-campo-radicacion/`
...
Run `opsxj:new SCRUM-51` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-51` only after the repo PR is merged.
