...
Use this template at the start of each Jira ticket (`SCRUM-55`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-55`
- Summary: `CREA-API-AUTO-COMPLETE-TOKEN-RADICADO`
- Coordinator change: `openspec/changes/scrum-55-crea-api-auto-complete-token-radicado/`
...
Run `opsxj:new SCRUM-55` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-55` only after the repo PR is merged.
