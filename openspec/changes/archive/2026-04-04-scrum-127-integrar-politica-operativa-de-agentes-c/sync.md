...
Use this template at the start of each Jira ticket (`SCRUM-127`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-127`
- Summary: `Integrar politica operativa de agentes Codex en flujo opsxj backend`
- Coordinator change: `openspec/changes/scrum-127-integrar-politica-operativa-de-agentes-c/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/164 | pending | in_review |
...
Run `opsxj:new SCRUM-127` only in rows with `Impacta? = yes`.
