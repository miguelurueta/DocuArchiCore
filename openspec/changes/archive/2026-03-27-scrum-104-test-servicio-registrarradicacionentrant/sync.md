...
Use this template at the start of each Jira ticket (`SCRUM-104`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-104`
- Summary: `TEST-SERVICIO-RegistrarRadicacionEntranteAsync`
- Coordinator change: `openspec/changes/scrum-104-test-servicio-registrarradicacionentrant/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/126 | pending | in_review |
...
Run `opsxj:new SCRUM-104` only in rows with `Impacta? = yes`.
