...
Use this template at the start of each Jira ticket (`SCRUM-62`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-62`
- Summary: `MIGRACIÓN .NET-csfc_verifica_existencia_radicado`
- Coordinator change: `openspec/changes/scrum-62-migraci-n-net-csfc-verifica-existencia-r/`
...
Run `opsxj:new SCRUM-62` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-62` only after the repo PR is merged.
...
## Migration Read-Only Repositories (MIGRACION-NET csfc_verifica_existencia_radicado)
