...
Use this template at the start of each Jira ticket (`SCRUM-57`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-57`
- Summary: `CREA-CONSULTA-CONFIGURACION-PLANTILLA`
- Coordinator change: `openspec/changes/scrum-57-crea-consulta-configuracion-plantilla/`
...
Run `opsxj:new SCRUM-57` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-57` only after the repo PR is merged.
