...
Use this template at the start of each Jira ticket (`SCRUM-50`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-50`
- Summary: `ACTUALIZACION-MENSAJE-VALIDACION-CAMPOS-RADICACION`
- Coordinator change: `openspec/changes/scrum-50-actualizacion-mensaje-validacion-campos/`
...
Run `opsxj:new SCRUM-50` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-50` only after the repo PR is merged.
