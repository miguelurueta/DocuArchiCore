...
- [x] 3.3 Resolver `DBT` desde `DA_EXTENSION.ESTADO_NORMAL` con normalizacion de extension (`PDF`/`.PDF`).
- [x] 3.4 Ajustar flujo de inventario para poblar `NOMBRE_AREA_DEPARTAMENTO`, `SERIE_DOCUMENTO`, `SUBSERIE_DOCUMENTO` con fallback de metadata TRD.
...
- [x] 3.7 Extender `StorageTransactionResult`/logging para trazabilidad de insercion gabinete e indice.

## 4. Compensation and Consistency

- [x] 4.1 Verificar/ajustar compensacion DB para eliminar registro de gabinete en fallo fisico post-commit.
- [x] 4.2 Verificar/ajustar compensacion de inventario/workflow/cuota/expediente ya existente.
- [x] 4.3 Definir y aplicar politica de compensacion para `ra_cert_indice_expediente` (delete o anulacion) consistente con arquitectura.

## 5. Tests

- [x] 5.1 Unit tests: insert gabinete obligatorio y rollback al fallar.
- [x] 5.2 Unit tests: mapeo `DBT` desde `DA_EXTENSION` para extension con/sin punto y case-insensitive.
- [x] 5.3 Unit tests: llenado TRD descriptivo con request y fallback metadata.
- [x] 5.4 Unit tests: reglas de insercion de indice electronico (aplica/no aplica/expediente inactivo).
- [x] 5.5 Integration tests: happy path `contabil` con fila DB + `DIG/FXL`.
- [x] 5.6 Integration tests: expediente valido inserta `ra_cert_indice_expediente` y mantiene consistencia.
- [x] 5.7 Integration tests: fallo fisico post-commit dispara compensacion y no deja huerfanos.
...
- [x] 6.2 Actualizar documentos base de arquitectura final de SCRUM-189 con la nueva secuencia y matriz de paridad.
...
- [x] 7.2 Confirmar tasks completas antes de `opsxj:orchestrate:publish`.
