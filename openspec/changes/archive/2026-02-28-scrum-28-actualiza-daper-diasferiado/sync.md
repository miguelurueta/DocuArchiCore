...
Use this template at the start of each Jira ticket (`SCRUM-28`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-28`
- Summary: `ACTUALIZA-DAPER-DIASFERIADO`
- Coordinator change: `openspec/changes/scrum-28-actualiza-daper-diasferiado/`
...
Run `opsxj:new SCRUM-28` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-28` only after the repo PR is merged.
