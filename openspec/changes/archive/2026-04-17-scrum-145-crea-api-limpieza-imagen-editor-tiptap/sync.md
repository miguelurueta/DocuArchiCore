...
Use this template at the start of each Jira ticket (`SCRUM-145`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-145`
- Summary: `CREA-API-LIMPIEZA-IMAGEN-EDITOR-TIPTAP`
- Coordinator change: `openspec/changes/scrum-145-crea-api-limpieza-imagen-editor-tiptap/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/191 | pending | in_review |
...
Run `opsxj:new SCRUM-145` only in rows with `Impacta? = yes`.
