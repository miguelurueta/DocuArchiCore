...
Use this template at the start of each Jira ticket (`SCRUM-29`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-29`
- Summary: `CREA-SERVICIO-API-SOLICITAFECHALIMITE-RESPUESTA`
- Coordinator change: `openspec/changes/scrum-29-crea-servicio-api-solicitafechalimite-re/`
...
Run `opsxj:new SCRUM-29` only in rows with `Impacta? = yes`.
Run `opsxj:archive SCRUM-29` only after the repo PR is merged.
