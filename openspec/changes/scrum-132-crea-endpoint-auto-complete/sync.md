...
Use this template at the start of each Jira ticket (`SCRUM-132`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-132`
- Summary: `CREA-ENDPOINT-AUTO-COMPLETE`
- Coordinator change: `openspec/changes/scrum-132-crea-endpoint-auto-complete/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/173 | pending | in_review |
...
Run `opsxj:new SCRUM-132` only in rows with `Impacta? = yes`.
