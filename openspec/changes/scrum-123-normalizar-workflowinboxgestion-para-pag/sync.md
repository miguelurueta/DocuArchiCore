...
Use this template at the start of each Jira ticket (`SCRUM-123`) to decide exactly where to run `opsxj:new` and `opsxj:archive`.

## Ticket

- Jira key: `SCRUM-123`
- Summary: `Normalizar workflowInboxgestion para paginaci?n consistente y claims reales`
- Coordinator change: `openspec/changes/scrum-123-normalizar-workflowinboxgestion-para-pag/`
...
| DocuArchiCore | yes | implementation_required | orquestador openspec central | done | https://github.com/miguelurueta/DocuArchiCore/pull/155 | pending | in_review |
...
Run `opsxj:new SCRUM-123` only in rows with `Impacta? = yes`.
